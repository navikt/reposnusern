...
var _ = Describe("NewConfig", func() {
	...
	It("aggregates setup validation errors instead of returning the first one", func() {
		...
		Expect(os.Setenv("REPOSNUSER_MAXDEBUGREPOS", "0")).To(Succeed())

		_, err := NewConfig()

		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("REPOSNUSERN_PARALL må være et positivt heltall"))
		Expect(err.Error()).To(ContainSubstring("REPOSNUSER_MAXDEBUGREPOS må være et positivt heltall"))
	...
	It("reports invalid max debug repos values", func() {
		Expect(os.Setenv("ORG", "navikt")).To(Succeed())
		Expect(os.Setenv("GITHUB_TOKEN", "token")).To(Succeed())
		Expect(os.Setenv("REPO_STORAGE", string(StoragePostgres))).To(Succeed())
		Expect(os.Setenv("POSTGRES_DSN", "postgres://user:pass@localhost:5432/reposnusern?sslmode=disable")).To(Succeed())
		Expect(os.Setenv("REPOSNUSER_MAXDEBUGREPOS", "not-a-number")).To(Succeed())
